...
public:
    ...
    {
        return static_cast<IndexType>(weights_.size());
    }
    IndexType incrementNumberOfWeights( ValueType val = 1 )
    {
        weights_.push_back(val);
        return numberOfWeights() - 1;
    }
    IndexType increaseNumberOfWeights( IndexType by_n = 1 )
    {
        weights_.resize(weights_.size() + by_n, 1.);
