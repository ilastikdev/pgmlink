...
public:
    ...
    size_t size() const
    {
        if(shape_.size())
        {
            return std::accumulate(shape_.begin(), shape_.end(), 1, Multiplier());
        }
        else
        {
            return 0;
    ...
    const std::vector<LabelType>& indicate() const
    {
        return indicate_;
    };
    void indicate_value( const ValueType& v )
    {
        value_ = v;
    }
    ValueType indicate_value() const
    {
        return value_;
    }
    void default_value( const ValueType& v )
    {
        default_ = v;
    }
