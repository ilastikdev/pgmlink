...
//	tracklet_tracklet_graph.add(node_track)
    ...
    num_of_nodes = 0;
    num_of_arcs = 0;
    num_of_tracklets_size[0] = 0;
    num_of_tracklets_size[1] = 0;
    num_of_tracklets_size[2] = 0;
...
BOOST_AUTO_TEST_CASE( HypothesesGraph_Map_Serialization )
{
...

    {
        ...
        HypothesesGraph::NodeIt node(*graph);
        ++node;
        HypothesesGraph::ArcIt arc(*graph);
        ++arc;
        ...
        std::vector<bool> arc_active_vec;
        arc_active_vec.push_back(false);
        arc_active_vec.push_back(true);
        ...
        std::vector<double> tracklet_dist_vec;
        tracklet_dist_vec.push_back(7.32);
...
BOOST_AUTO_TEST_CASE( HypothesesGraph_SubGraph_Copy )
{
    ...
    // verify
    {
        HypothesesGraph::NodeIt node(*other);
        ++node;
        HypothesesGraph::ArcIt arc(*other);
        ++arc;
