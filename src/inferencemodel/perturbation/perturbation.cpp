...
double Perturbation::get_transition_variance(Traxel& tr1,
                                             Traxel& tr2,
                                             boost::shared_ptr<InferenceModel::TransitionPredictionsMap> transition_predictions)
{
    ...
    {
        InferenceModel::TransitionPredictionsMap::const_iterator it = transition_predictions->find(std::make_pair(tr1, tr2));
...
double Perturbation::sigmoid(double x)
{
    return cdf(standard_gaussian_distribution, x);
}

double Perturbation::inverse_sigmoid(double x)
{
    if (x <= 0.000001)
    {
        x = 0.000001;
...
double Perturbation::sample_with_classifier_variance(double mean, double variance)
{
...
double Perturbation::inverseWeightedNegLog(double energy, double weight)
{
    assert (weight != 0);
    return exp(-energy / weight);
}

double Perturbation::weightedNegLog(double prob, double weight)
{
    if (prob <= 0.00001)
    {
        prob = 0.00001;
...
double Perturbation::generateRandomOffset(EnergyType energyIndex,
                                          double energy,
                                          Traxel tr,
                                          Traxel tr2,
                                          size_t state,
